...
function sortTradesChronologically(
    ...
    return [...trades].sort(
        (a, b) =>
            new Date(a[8]).getTime() -
            new Date(b[8]).getTime()
    );
}

export function buildCumulativeRData(
    trades: Trade[]
) {
    let cumulativeR = 0;
...
export function buildCumulativePnlData(
    trades: Trade[]
) {
    let cumulativePnl = 0;
